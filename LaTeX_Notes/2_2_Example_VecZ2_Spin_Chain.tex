...
We will now introduce defects to this model, indicated by red lines that fuse to the chain, e.g.\ a chain with one defect would be
	...
	\end{figure}
	\noindent
Since the chain is represented by a category, $\Vec(\Z/2)$ in our example, the defect is a $\Vec(\Z/2)-\Vec(\Z/2)$ bimodule, i.e.\ we are fusing an object from the bimodule to the chain. Here, we will use the bimodule $F_1$ to introduce defects to the $\Vec(\Z/2)$-chain. $F_1$ has only one object, so we omit writing labels for the bimodule object, but indicate by a red line when the object is from the bimodule.
